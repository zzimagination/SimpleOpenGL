...
{
	GraphicVertexData::GraphicVertexData(RenderVertexData* renderVertex)
	{
		VAO = 0;
		VBO = 0;
		EBO = 0;
	...
	void GraphicVertexData::BindData(Vector3* vertices, int count, int* index, Vector2* uv)
	{
		if (count >= 4)
		{
	...
	int GraphicVertexData::GetCount()
	{
		return _pointCount;
	}

	void GraphicVertexData::UseData()
	{
		glBindVertexArray(VAO);
	}
}